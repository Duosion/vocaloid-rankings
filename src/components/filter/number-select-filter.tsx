...
export function NumberSelectFilterElement(
    ...
    for (let i = start; i < end; i += increment) {
        ...
        if (num == value) currentOption = index;
        index++
    }

    return (
        <SelectFilterElement searchable name={name} value={currentOption} defaultValue={defaultValue} options={options} elevation={elevation} modalElevation={modalElevation} onValueChanged={newValue => { if (onValueChanged) onValueChanged(optionsReverseMap[newValue]) }} />
    )
