...
export function RankingsGridItem(
    ...
    return (
        <article key={key} className={`w-full flex flex-col gap-3 relative box-border items-center transition-all ${className}`} style={{
            ...
            <Link href={href} className="max-w-full"><h3 className="overflow-clip text-ellipsis text-on-surface transition-colors hover:text-inherit text-center font-semibold text-2xl max-h-[3lh] line-clamp-3" title={iconAlt}>{titleContent}</h3></Link>
